...
const blockFile = path.join(__dirname, 'block.txt');
...
class PeerManager extends EventEmitter {
  ...
  constructor() {
    ...
    this._rlpx = new devp2p.RLPx(PRIVATE_KEY, {
      dpt: dpt,
      maxPeers: 100,
  ...
  _init() {
    ...
    const rlpx = this._rlpx;


    const txCache = new LRUCache({ max: 1000 })
    function onNewTx(tx: Tx, peer: Peer) {
    ...
    async function startSync() {
      while(true) {
        if (blockNumberCache[currentBlockHeight + 1]) {
          currentBlockHeight = currentBlockHeight + 1;
          continue;
        }

        const blockHash = blockNumberCache[currentBlockHeight];
        await sleep('10s');
        const keys = Object.keys(peers);
        let index = 0;
        for (let i = 0; i < keys.length; i++) {
           peers[keys[i]](currentBlockHeight);
           index++;
           if (index >= 3) {
             break;
           }
    ...
    const blockNumberCache: { [key:string]:Buffer; } = {};
    ...
    function onNewBlock(block: Block, peer: Peer) {
      ...
      blockNumberCache[String(blockNumber)] = block.hash();
      blocksCache.set(blockHashHex, true)
      console.log(
        `----------------------------------------------------------------------------------------------------------`,
      )
      console.log(`New block ${blockNumber}: ${blockHashHex} (from ${getPeerAddr(peer)})`)
      fs.appendFileSync(blockFile, `${blockNumber}: ${blockHashHex}\n`)
      // console.log(JSON.stringify(block.toJSON(true), null, 2))
      console.log(
        `----------------------------------------------------------------------------------------------------------`,
      )
    ...
    rlpx.on('peer:added', peer => {
      ...
      peers[addr] = function requestBlock(blockHash: any) {
        eth.sendMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_HEADERS, [ blockHash, 2, 0, 0 ])
      }
        ...
        switch (code) {
          ...
          case devp2p.ETH.MESSAGE_CODES.BLOCK_HEADERS:
            if (!forkVerified) {
              ...
              const header = new Block.Header(payload[0])
          ...
          case devp2p.ETH.MESSAGE_CODES.BLOCK_BODIES:
            ...
            while (requests.bodies.length > 0) {
              ...
              const block = new Block([header.raw, payload[0][0], payload[0][1]])
    ...
    rlpx.on('peer:error', (peer, err) => {
      ...
      if (err instanceof assert.AssertionError) {
        ...
        if (peerId !== null) {
          this._dpt.banPeer(peerId, ms('5m'))
        }
